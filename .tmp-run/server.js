...
import { activateKillSwitch, approveDraft, createPipelineServices, enableApprovalPhase, initializeServices, nextRetryIso, processJob, recordDeadLetter, rejectDraft, rerunValidationForExecution, refineValidationForExecution, resumeAutomation, runDiscoveryCycle, sendCeoMessage, simulateSupplierFollowUp, } from './src/lib/pipeline';
...
function buildAgentFleet(state) {
...
function buildStoreOverview(state) {
    ...
    return [
        {
            ...
            note: ebayConnection?.status === 'sandbox'
...
async function buildAgentsPayload() {
    const state = await getState();
    return {
        generatedAt: new Date().toISOString(),
...
async function buildSupplierChatsPayload() {
    const state = await getState();
    return {
        generatedAt: new Date().toISOString(),
...
async function buildCustomerChatsPayload() {
    const state = await getState();
    const ebayConnected = Boolean(loadConfig().ebay.clientId || loadConfig().ebay.sandboxClientId);
    return {
        generatedAt: new Date().toISOString(),
...
async function buildCeoChatPayload() {
    const state = await getState();
    return {
        generatedAt: new Date().toISOString(),
...
async function buildArchitecturePayload() {
    const state = await getState();
    return {
        generatedAt: new Date().toISOString(),
...
async function startServer() {
    ...
    app.get('/api/agents/:agentId/contract', async (req, res) => {
        ...
        res.json({
            generatedAt: new Date().toISOString(),
            contract: getAgentContract(state, req.params.agentId),
        });
    });
