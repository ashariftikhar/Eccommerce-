...
import { appendCeoMessage, appendSupplierMessage, createStoreAdapter, finishExecution, pushAudit, pushExecutionStep, pushRun, pushValidationRun, startExecution, } from './store';
import { clamp, createId, isMilitaryAddress, isPoBox, normalizeKeyword, nowIso, sha256, shortHash, withJitter } from './utils';
import { findLatestValidationForExecution, validateCjMatch, validateFulfillment, validateListingGenerator, validateManager, validateProductScoring, validatePublish, validateSupplierLiaison, validateTrendDiscovery, } from './validation';
export function createPipelineServices(config) {
    return {
        config,
...
function addExecutionLink(target, executionId) {
...
function prefilterProduct(product, state) {
...
function riskClassForCandidate(candidate) {
...
export async function runDiscoveryCycle(services) {
    ...
    for (const [index, candidate] of shortlisted.entries()) {
        const policy = evaluatePolicy({
            ...
            tags: candidate.tags,
        }, state.prohibitedRules, state.veroBlocklist);
        ...
        if ((candidate.status === 'READY_TO_LIST' || candidate.status === 'TEST_ONLY') && !state.listingDrafts.some((draft) => draft.candidateId === resolvedCandidate.id)) {
            ...
            if (categoryPolicies.valid) {
                ...
                drafted += 1;
                pushValidationRun(state, validateListingGenerator(listingExec.id, draft, {
...
export async function approveDraft(services, draftId, actor = 'owner') {
    const state = await services.store.getState();
...
export async function rejectDraft(services, draftId, actor = 'owner') {
    const state = await services.store.getState();
    const draft = state.listingDrafts.find((item) => item.id === draftId);
    if (!draft) {
        throw new Error('Draft not found.');
...
export async function publishDraftNow(services, draftId) {
    const state = await services.store.getState();
    const draft = state.listingDrafts.find((item) => item.id === draftId);
    if (!draft) {
        throw new Error('Draft not found.');
...
export async function enableApprovalPhase(services) {
    const state = await services.store.getState();
...
export async function resumeAutomation(services) {
    const state = await services.store.getState();
...
export async function activateKillSwitch(services) {
    const state = await services.store.getState();
...
export function lowRiskOrderReasons(order, state, candidate) {
    const reasons = [];
...
export async function simulateSupplierFollowUp(services, chatId, ownerMessage) {
    const state = await services.store.getState();
    ...
    const chat = appendSupplierMessage(state, chatId, {
        id: createId('msg'),
        sender: 'owner',
        direction: 'outbound',
...
export async function sendCeoMessage(services, prompt) {
    const state = await services.store.getState();
    ...
    const reply = await services.ai.generateCeoReply(state, `${prompt} Current system state: ${state.candidates.length} candidates, ${state.listingDrafts.length} drafts, ${state.orders.length} orders, mode ${state.settings.automationMode}.`);
...
export async function processJob(services, job) {
    switch (job.type) {
        case 'discovery':
...
export async function rerunValidationForExecution(services, executionId) {
    const state = await services.store.getState();
    ...
    switch (latest.agentId) {
        ...
        case 'listing_generator': {
            ...
            if (!draft)
                throw new Error('Draft not found.');
            pushValidationRun(state, validateListingGenerator(executionId, draft, {
        ...
        case 'fulfillment': {
            ...
            const candidate = state.candidates.find((item) => item.id === draft?.candidateId);
            const reasons = lowRiskOrderReasons(order, state, candidate);
            pushValidationRun(state, validateFulfillment(executionId, order, fulfillment, reasons, Boolean(fulfillment?.cjOrderId), state.connections.find((item) => item.name === 'CJdropshipping')?.status !== 'connected'));
            break;
        }
        ...
        case 'manager': {
            ...
            if (!message)
                throw new Error('CEO message not found.');
...
export async function refineValidationForExecution(services, executionId) {
    const state = await services.store.getState();
    const latest = findLatestValidationForExecution(state, executionId);
    if (!latest) {
        throw new Error('Validation execution not found.');
...
export async function recordDeadLetter(services, job, error) {
    const state = await services.store.getState();
...
export function nextRetryIso(attemptCount) {
