...
function competitionScore(snapshot) {
    return clamp(Math.round(100 * (1 - Math.min(3, snapshot.competitionRatio) / 3) * (1 - 0.5 * snapshot.eliteShare)), 0, 100);
}
function shippingSpeedScore(eta) {
    if (eta <= 3) {
        return 100;
    }
    if (eta <= 5) {
...
function returnRiskScore(product) {
...
function supplierReliabilityScore(product) {
    const supplierAgeScore = Math.min(100, product.supplierAgeDays / 7);
    return round(clamp(supplierAgeScore * 0.2 + product.shippingConsistency * 0.35 + product.stockStability * 0.2 + product.orderHistoryScore * 0.25, 0, 100), 0);
}
function listingComplexityScore(product) {
    const haystack = `${product.title} ${product.categoryPath} ${product.tags.join(' ')}`.toLowerCase();
    let score = 100;
    const penalties = [
        [/\bset of\b/, 10],
...
function priceForMargin(product) {
...
export function scoreCandidate(args) {
    ...
    const totalScore = round(demand * 0.22 +
        shipping * 0.18 +
        margin * 0.2 +
        competition * 0.1 +
        reliability * 0.1 +
