import { CJProduct, PolicyState, ProhibitedRule, VeroRule } from '../types';
import { createId, normalizeKeyword } from './utils';

export function seedProhibitedRules(): ProhibitedRule[] {
  return [
...
export function seedVeroRules(): VeroRule[] {
  return [
...
export function evaluatePolicy(product: CJProduct, prohibitedRules: ProhibitedRule[], veroRules: VeroRule[]): PolicyEvaluation {
  const haystack = normalizeKeyword(`${product.title} ${product.categoryPath} ${product.tags.join(' ')}`);
  const matches: string[] = [];
  let state: PolicyState = 'clear';
