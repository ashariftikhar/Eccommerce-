...
function computeRunStatus(results: ValidatorResult[]): { status: ValidationStatus; blocking: boolean; score: number } {
...
export function buildValidationRun(input: ValidationInput): ValidationRun {
...
export function summarizeValidationRuns(state: AppState): Array<{ agentId: AgentId; passed: number; warning: number; failed: number }> {
...
export function architectureFlowchart(): string {
  return [
    'flowchart TD',
...
export function architectureRoutingRules(): string[] {
  return [
...
export function architectureFailureMatrix(): Array<{ id: string; stage: string; failure: string; handling: string }> {
  return [
...
export function validateTrendDiscovery(executionId: string, scanned: number, shortlisted: number, usedSynthetic: boolean): ValidationRun {
...
export function validateCjMatch(executionId: string, candidate: ProductCandidate, simulated: boolean): ValidationRun {
  return buildValidationRun({
    agentId: 'cj_match',
    executionId,
...
export function validateProductScoring(executionId: string, candidate: ProductCandidate, simulatedMarket: boolean): ValidationRun {
  ...
  return buildValidationRun({
    ...
    validatorResults: [
      ...
      makeValidator(
        'total-range',
        'Total score in range',
        'hard',
        typeof score?.totalScore === 'number' && score.totalScore >= 0 && score.totalScore <= 100,
        'Total score between 0 and 100',
        score ? `${score.totalScore}` : 'missing',
      ),
      makeValidator(
        'decision-consistency',
        'Decision consistency',
        'hard',
        !score || !(score.decision === 'READY_TO_LIST' && score.hardRejectReasons.length > 0),
        'READY_TO_LIST should not include hard reject reasons',
        score ? `${score.decision} / rejects ${score.hardRejectReasons.length}` : 'missing',
      ),
...
export function validateListingGenerator(
  executionId: string,
  draft: ListingDraft,
  options: { fallbackUsed: boolean; simulatedPolicy: boolean; categoryWarnings: string[]; refinementCount?: number },
): ValidationRun {
  return buildValidationRun({
...
export function validatePublish(executionId: string, draft: ListingDraft, simulated: boolean, duplicateDetected = false): ValidationRun {
  return buildValidationRun({
...
export function validateFulfillment(
  executionId: string,
  order: OrderRecord,
  fulfillment: FulfillmentJob | null,
  reasons: string[],
  placed: boolean,
  simulated: boolean,
): ValidationRun {
...
export function validateSupplierLiaison(
  executionId: string,
...
export function validateCustomerSupportDraft(
  executionId: string,
  resourceId: string,
  draftMessage: string,
  fallbackUsed: boolean,
...
export function getAgentContract(state: AppState, agentId: AgentId): AgentContract | null {
  return state.agentContracts.find((contract) => contract.agentId === agentId) || null;
}

export function findLatestValidationForExecution(state: AppState, executionId: string): ValidationRun | null {
